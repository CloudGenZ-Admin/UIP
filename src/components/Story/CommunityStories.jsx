          ...
          {STORIES.map((story, idx) => (
            <div key={idx} className="bg-gray-50 rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-md transition-all group flex flex-col">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                {/* Small Form Image/Avatar Container */}
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-sm transform group-hover:scale-105 transition-transform flex-shrink-0 ${!story.img ? `bg-gradient-to-br ${story.gradient}` : ''}`}>
                  {story.img ? (
                    <img 
                      src={story.img} 
                      alt={story.name} 
                    ...
                    story.emoji // Fallback agar image na ho
                  )}
                </div>
