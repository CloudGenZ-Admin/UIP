    ...
    <div className="p-4 md:p-6 lg:p-10 pt-20 md:pt-10 max-w-7xl mx-auto h-full flex flex-col relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="bg-white px-5 py-2.5 rounded-xl shadow-sm border border-slate-200 font-bold text-slate-700 flex items-center gap-2">
          ...
          <span className="bg-[#3B82F6] text-white px-2 py-0.5 rounded-md text-sm">{data.length}</span>
        </div>
      </div>
              ...
              {currentItems.map((item) => (
                <div key={item.id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-4">
                  ...
                  <div className="flex justify-between items-start gap-4 border-b border-slate-50 pb-4">
                    ...
                    <span className="shrink-0 text-[#3B82F6] font-bold bg-[#3B82F6]/10 px-3 py-1.5 rounded-md text-xs text-center max-w-[100px]">
                      {item.session_type}
                    </span>
                  </div>
                  ...
                  <div className="text-sm text-slate-600 bg-slate-50 p-4 rounded-2xl space-y-2">
                    <div className="line-clamp-1"><strong className="text-slate-800">Challenges:</strong> {item.challenges || 'N/A'}</div>
                    <div className="line-clamp-1"><strong className="text-slate-800">Goals:</strong> {item.goals || 'N/A'}</div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-4xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-2xl font-black text-slate-800">{selectedWellness.name}</h2>
                <p className="text-slate-500 text-sm font-medium mt-1">Full Wellness Booking Details</p>
              </div>
              <button 
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="bg-slate-50 px-4 py-3 rounded-xl text-slate-800 text-sm">{selectedWellness.email}</div>
                  </div>
                  <div>
