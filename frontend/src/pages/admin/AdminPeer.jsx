...
export default function AdminPeer() {
  ...
  // State to hold the currently selected peer for the modal
  const [selectedPeer, setSelectedPeer] = useState(null);

  // Pagination States
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 4; // Show exactly 4 registrations per page
  ...
  const handleDelete = async (id) => { 
    if(window.confirm('Are you sure you want to delete this registration?')) { 
      try {
        await apiService.deletePeerSupport(id); 
    ...
    <div className="p-4 md:p-6 lg:p-10 pt-20 md:pt-10 max-w-7xl mx-auto h-full flex flex-col relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="bg-white px-5 py-2.5 rounded-xl shadow-sm border border-slate-200 font-bold text-slate-700 flex items-center gap-2">
          <span>Total:</span> 
          <span className="bg-[#A855F7] text-white px-2 py-0.5 rounded-md text-sm">{data.length}</span>
        </div>
      </div>
                  ...
                  <div className="text-sm text-slate-600 bg-white border border-slate-100 p-4 rounded-2xl">
                    ...
                    {item.access_needs && (
                      <div className="pt-2 border-t border-slate-100 mt-2">
                        ...
                        <p className="text-orange-600 text-xs font-medium">{item.access_needs}</p>
                      </div>
                    )}
                  </div>
      ...
      {selectedPeer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-2xl font-black text-slate-800">{selectedPeer.name}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-slate-50 px-4 py-3 rounded-xl text-slate-800 text-sm">{selectedPeer.email}</div>
                </div>
                <div>
              ...
              <div className="col-span-1 md:col-span-2 space-y-4 mt-2 pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="bg-orange-50 px-4 py-3 rounded-xl text-orange-800 text-sm leading-relaxed border border-orange-100">
                  ...
                  <div>
                    ...
                    <div className="bg-red-50 px-4 py-3 rounded-xl text-red-800 text-sm leading-relaxed border border-red-100">
